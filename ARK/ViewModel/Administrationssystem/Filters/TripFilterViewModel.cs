...
namespace ARK.ViewModel.Administrationssystem.Filters
{
    public class TripFilterViewModel : FilterViewModelBase, IFilter
    {
        ...
        public TripFilterViewModel()
        {
            ...
            {
                AddCheckBox(boattype.ToString(), (trips) => trips.Where(trip => trip.Boat.SpecificBoatType == boattype), ControlsBoatType);
            }
        ...
        {
            AddCheckBox(text, (trips) => trips.Where(trip => trip.Distance >= from
                                                             && trip.Distance <= to), targetList);
        }

        private void AddCheckBox(string text, Func<IEnumerable<Trip>, IEnumerable<Trip>> func, ICollection<CheckBox> targetList)
        {
        ...
        public IEnumerable<T> FilterItems<T>(IEnumerable<T> items)
        {
            ...
            return boatTypes.Where(trip => distance.Any(trip2 => trip == trip2)
                                           && year.Any(trip2 => trip == trip2))
                            .OrderByDescending(trip => trip.TripStartTime).Cast<T>();
        }
    }
}